...
Questions and concerns have been raised since IETF 122 presentation regarding whether RST with payload affects the normal termination of flows across different software platforms, operating systems, middleboxes, etc,. Even though RFC 9293 Section 3.5.3. Reset Processing explicitly allows this behavior, a full implementation is still needed to widely verify if unexpected cases can happen in the real world.
...
* **Hardwares and Kernels:** Tests were conducted on various Linux distributions (e.g., Ubuntu, CentOS) with different kernel versions. The physical hosts were equipped with a range of network interface cards (NICs), including Intel `i40e`, `ixgbe`, and Mellanox `mlx5`.
...
* **Middle Boxes:** Tests were performed with Layer 4 (L4) and Layer 7 (L7) gateways placed between the client and server to verify correct packet parsing and forwarding.
* **Wide Area Network:** The setup was tested over long-haul international links to simulate complex conditions, including China-to-Singapore (RTT > 30ms) and China-to-Germany (RTT > 200ms).
